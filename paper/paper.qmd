...
subtitle: "Kamala Harris Predicted to Lead with 341 Electoral Votes Based on a Poll-Based Prediction"
author: 
  - Yuanyi (Leo) Liu
  - Dezhen Chen
  - Ziyuan Shen
...
The structure of the paper is organized as follows: following @sec-intro, @sec-data presents the data collection and cleaning process, along with an overview of the variables used in the analysis. @sec-model introduces the forecasting models, explaining why the selected models are suitable for predicting election outcomes based on aggregated polling data. Then, @sec-result provides the results, highlighting key trends and predictions. Eventually, @sec-discussion concludes with a discussion of the findings, evaluating the reliability of the forecasts and identifying potential limitations of the models.
...
ggplot(cleaned_data, aes(x = end_date, y = pct)) +
  ...
  geom_smooth(color = "#FF6F61", fill = "#FF6F61", method = "loess", se = TRUE, linetype = "solid", size = 1.2) + 
  ...
  theme(
    ...
    axis.text = element_text(color = "gray20"),  # Darker axis text for contrast
    panel.grid.minor = element_blank()  # Remove minor grid for a cleaner look
)
```

Using package `ggplot2` [@citeggplot], @fig-trump-pct shows Donald Trump's polling percentages over time from July to October 2024. Each black dot represents an individual poll conducted on a specific date, indicating the percentage of voters who supported Trump in that poll. Initially, Trump's polling percentage appears stable with small fluctuations around 45%. However, as the campaign progresses into October, there is a slight upward trend in Trump's polling percentage, suggesting that his support increased as the election neared.
...
```{r}
#| label: tbl-trump-summary
#| tbl-cap: "Summary Statistics of Trump's Support Percentage in 2024 Polling Data"
#| echo: false
#| warning: false
...
summary_table <- cleaned_data %>%
  summarize(
    ...
    Std_Dev = sd(pct, na.rm = TRUE),
    Count = n()
  )

kable(summary_table,
      col.names = c("Mean", "Median", "Min", "Max", "Standard Deviation", "Count"),
      digits = 2)
```

@tbl-trump-summary presents the key summary statistics for Trump’s support percentage based on a dataset of 348 polling observations from the 2024 U.S. presidential election. The mean support level is 46.47%, indicating that, on average, 46.47% of respondents in the polls expressed support for Trump. The median value of 47% suggests that half of the polls reported support at or below 47%, while the minimum and maximum values show a wide range of support from 27% to 57.9%, reflecting variability across different polls and regions. The standard deviation of 4.73 indicates a moderate spread around the mean, suggesting some diversity in polling results but with a central clustering.
...
@tbl-summary presents the coefficients from our model analyzing factors that influence polling outcomes. The intercept value, 44.178, represents the baseline level of support when all other predictors are at their reference or baseline levels. Key variables include "numeric_grade," with a positive coefficient indicating that higher grades correlate with increased support. Different polling methodologies show varying effects; for example, "Mixed Voting" has a positive coefficient of 1.697, while "Online Panel" and "Probability Panel" methods have negative coefficients, suggesting that these methods might be associated with lower support levels in polling outcomes.

```{r}
#| label: tbl-summary
#| tbl-cap: "Coefficients from a regression model examining factors influencing polling outcomes, including polling methodology, duration, transparency, and state-level indicators."
#| echo: false
#| warning: false
