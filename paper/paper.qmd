...
date: today
date-format: long
abstract: "In this paper, we have built a linear model to predict the outcome of the 2024 U.S. presidential election, using a \"poll-of-polls\" approach that combines data from many well-known pollsters. Our analysis shows that Donald Trump has a probability of winning in range 45% to 55% in each state, which depends on how polling data changes from state to state. However, Kamala Harris is projected to win key states and receive the majority of electoral votes, with a projected 239 votes to Trump's 98, based on testing dataset, we only have those state in the dataset. These show the value of aggregate polls in providing reliable predictions of election outcomes."
format: pdf
...
The results of presidential elections influence national and international policies and determine governance and economic priorities. An accurate election forecast is a valuable tool for political strategists, the media, and the public. The 2024 U.S. presidential election is expected to be highly competitive, with Donald Trump and Kamala Harris as the leading candidates. In this paper, we use national polling data from the period leading up to the 2024 U.S. presidential election to generate a linear regression model that predicts the winner while capturing changes in public opinion throughout the election period.

This study estimates the probability of each candidate winning based on aggregate polling data. By combining the results, we reduce the bias and short-term volatility of individual polls. By using a linear model, we estimate the probability of a Trump victory to be between 45% and 55%, which depends on state-by-state variations.
...
## Prediction map
@fig-statemap displays the predicted outcome for each state in the 2024 U.S. Presidential Election, with green representing states where Trump is expected to win and blue for those where Harris is predicted to prevail.
```{r}
#| label: fig-statemap
